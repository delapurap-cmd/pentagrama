...
(async()=>{let browser;
 ...
 for(const viewport of [{width:1440,height:900},{width:390,height:844},{width:320,height:720}]){
  ...
  assert.equal((await number.textContent()).trim(),'1','System number is only numeral');
  ...
  console.log("TAB POINTER DIAGNOSTIC",await page.locator('.linked-tab [data-tab-add="0"][data-tab-string="1"]').first().evaluate(el=>{const r=el.getBoundingClientRect();const at=document.elementFromPoint(r.left+r.width/2,r.top+r.height/2);return {target:r.toJSON(),at:at?.outerHTML.slice(0,220),svg:el.ownerSVGElement.getBoundingClientRect().toJSON(),sheet:el.closest(".sheet").getBoundingClientRect().toJSON(),events:getComputedStyle(el).pointerEvents}}));
  ...
  assert.equal(await page.locator('.linked-tab .tab-cell').first().textContent(),'0');
  ...
  const rendered=await page.locator('#stage .linked-tab').first().locator('.tab-cell text').allTextContents();
