...
(async()=>{let browser;try{
 ...
 for(const viewport of [{width:1440,height:900},{width:390,height:844},{width:320,height:720}]){
  ...
  await page.locator('#ppRangeButton').click();
  assert.equal(await page.locator('#btnMetroAlways svg.metro-icon').count(),1,'Physical metronome icon always visible');
  await page.locator('#btnMetroAlways').click();
  assert.equal(await page.locator('#btnMetroAlways').getAttribute('aria-pressed'),'true');
  assert.equal(await page.evaluate(()=>Sound.metroOn()),false,'Armed clock MUST NOT sound before Play');
  await page.waitForTimeout(350);
  assert.equal(await page.evaluate(()=>Sound.metroOn()),false,'No standalone metronome after waiting');
  await page.locator('#ppPlay').click();
  await page.waitForFunction(()=>Sound.playing());
  assert.equal(await page.locator('#btnMetroAlways').getAttribute('aria-pressed'),'true','Metronome stays armed during play');
  await page.locator('#ppPlay').click();
  assert.equal(await page.evaluate(()=>Sound.playing()),false,'Pause stops score playback');
  assert.equal(await page.evaluate(()=>Sound.metroOn()),false,'Pause must be SILENT, not free-running');
  assert.equal(await page.locator('#btnMetroAlways').getAttribute('aria-pressed'),'true','Still armed for next Play');
  await page.locator('#btnMetroAlways').click();
  assert.equal(await page.locator('#btnMetroAlways').getAttribute('aria-pressed'),'false','Disarm metronome');
