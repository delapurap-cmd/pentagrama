...
(async()=>{let browser;try{
  ...
  const realResponse=page.waitForResponse(r=>r.url().includes('/api/omr/jobs/')&&r.url().endsWith('/result')&&r.status()===200,{timeout:220000});
  ...
  assert.ok(outcome.notes>0&&outcome.measures>0&&outcome.backup,'real result must include editable notes and backup');
